...
Complete the `RLMFilesystemRAG` class by wiring all components together and implementing the main `prepare_documents()` and `query()` methods.

## Dependencies

- Task 1-5 all completed

## Files to Modify

1. `src/custom_rag/rlm/rlm_rag.py` - Complete the stub implementations
2. `src/custom_rag/rlm/__init__.py` - Final exports

## Context

This task completes the implementation by:
1. Implementing `prepare_documents()` to process and index documents
2. Implementing `query()` to route to either SimpleContextRAG or RLMAgent
3. Implementing `query_stream()` for streaming support
4. Adding proper error handling and logging

## Implementation

### File: `src/custom_rag/rlm/rlm_rag.py` (Complete Version)
...
if TYPE_CHECKING:
    from custom_rag.base_rag import RAGConfig
...
class RLMFilesystemRAG:
    ...
    Example:
        ...
        # Prepare documents (creates indexes and summaries)
    ...
    def __init__(
        ...
        Args:
            config: Base RAG configuration (optional, for compatibility)
    ...
    def prepare_documents(self, documents_path: str, force: bool = False) -> dict[str, Any]:
        ...
        if not force and self._manifest.is_valid(input_dir):
            ...
            self._prepared_path, prep_metrics = processor.prepare(str(input_dir))
...
### File: `src/custom_rag/rlm/__init__.py` (Final Version)

```python
"""RLM Filesystem RAG - Recursive Language Model approach for large corpora.

This package implements an RLM-style RAG that treats the document corpus as
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    (docs_dir / 'ml_basics.md').write_text('''# Machine Learning Basics
...
''')
    ...
    (docs_dir / 'deep_learning.md').write_text('''# Deep Learning

Deep learning uses neural networks with many layers.

## Neural Networks
Composed of neurons organized in layers.
...
''')
    ...
    # Test with lite mode (no API calls for prep if use_llm_* is False)
    config = RLMConfig(
        security_mode='lite',
        use_llm_summaries=False,
        use_llm_topics=False,
        small_corpus_threshold=5,  # Force RLM mode for testing
    )
    ...
    print(f'Prepared: {metrics}')

    # Check metrics
    full_metrics = rag.get_metrics()
    print(f'Mode: {full_metrics[\"mode\"]}')
    print(f'Documents: {full_metrics[\"total_documents\"]}')

    # Note: query() would require OPENAI_API_KEY
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    with RLMFilesystemRAG(rlm_config=config) as rag:
        rag.prepare_documents(str(docs))
        print(f'Metrics: {rag.get_metrics()[\"total_documents\"]} docs')
...
Proceed to `TASK_7_tests.md` to implement unit tests for all components.
